...
report_name, start, end, cropped =info['Results_name'], info['start'], info['end'], info["crops"]
...
with open(basefile, "r+") as apsimx:
          app_base = json.load(apsimx) 
apsim_save = os.path.join(os.getcwd(), "Your_apsimxfile")
edited_basefile = os.path.join(apsim_save, "edited_base_file.apsimx")
if not os.path.exists(apsim_save):
  os.mkdir(apsim_save)
apsim_file = None
apsim_file = json.dumps(app_base)
with open(edited_basefile, "w+")  as openfile:        
        openfile.write(apsim_file)
...
for crops in cropped:
  ...
  def save_simulation_results(result_list):
    ...
    results_csv = os.path.join(base_sim_path, crops+ ".csv")
