...
def DownloadsurgoSoiltables(lonlat, select_componentname =None, summarytable=False):
    ...
    response = requests.post(url,data=body,headers=headers, timeout =30.0)
...
def soilvar_perdep_cor(nlayers, soil_bottom =200, a = 0.5, b= 0.5):
         ...
         if a < 0:
               print("Target parameter can not be negative")
         elif (a > 0 and  b != 0):
            ep = -b*depthn
...
class OrganizeAPSIMsoil_profile:
  # Iinitiate the soil object and covert all into numpy array and change them to floating values
      def __init__(self, sdf, thickness, bottomdepth = 200):
      ...
      def variable_profile(self, y, kind = 'linear'):
            ...
            yinterpo  = interpolate.interp1d(x, y, kind = kind, assume_sorted=False)
            ...
            # dfl = {"depth": Depth, 'xnew': xnew, 'ynew': apsimvar}
            # dfnew = pd.DataFrame(dfl)
            # return dfnew
            
      def cal_satfromBD (self):
      ...
      def cal_dulFromsand_clay_OM(self):
            clay =  self.clay * 0.01
            sand  = self.sand * 0.01
            om  = self.OM * 0.01
            ...
            dulc  = soil.variable_profile(dul)
            return dulc
      def cal_l15Fromsand_clay_OM(self):
            clay =  self.clay * 0.01
            sand  = self.sand * 0.01
            om  = self.OM * 0.01
            ...
            return l151  
      def calculateSATfromwat_r(self):
      ...
      def cal_missingFromSurgo(self, curveparam_a = 0, curveparam_b = 0.2, crops = ["Wheat", "Maize", "Soybean", "Rye"], metadata = None, soilwat = None, swim = None, soilorganicmatter = None):
            ...
            FInert = 0.83 * soilvar_perdep_cor(nlayers, a = curveparam_a, b = -0.01) 
